...
class AttentionLayer(nn.Module):
    def __init__(self, d_model):
        ...
        self.attn = nn.Linear(d_model + d_model, d_model)
        self.w_v = nn.Linear(d_model, 1) # this attention is per-timestep calculation
        
    def forward(self, hidden_states, encoded_features):
        """
        ...
        correlation = torch.tanh(self.attn(torch.cat((hidden_states, encoded_features), dim=-1))) # (w, bs, d_model)
...
class Decoder(nn.Module):
    ...
    def forward(self, inputs, hidden_states, cell_states, encoder_outputs):
        ...
        attn_weights = self.attention(hidden_states, encoder_outputs) # (bs, 1, w)
